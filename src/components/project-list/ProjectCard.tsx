...
const ProjectCard = ({ project, activeState, index }:Props) => {
  ...
  return (
    ...
    >
      <div
        className={`card-wrapper relative h-[32rem] w-full max-w-[600px] shadow-lg bg-base-400 p-8 transition-all ${active === project.name ? 'card-toggle' : ''} overflow-hidden rounded-sm`}
      >
        <div className={`card-front z-[2] absolute bg-base-200 w-full h-full top-0 left-0 ${active === project.name ? 'translate-x-[100%]' : 'translate-x-[0]'} p-4`}>
          <img  src={project.image} alt={project.name} width='300px' height='300px' className="w-[95%] h-[35%] lg:h-[55%] mx-auto object-contain mb-1" />
          <div className="h-[40%]">
            ...
            <ProjectBtns link={project.link} deployed={project.deployed} handleSetActive={handleSetActive} />
          </div>
        </div>
