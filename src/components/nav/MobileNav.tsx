...
const MobileNav = ({ open, setOpen }:Props) => {
  ...
  return (
    <>
      <div className={`flex flex-col gap-1 md:hidden absolute left-0 w-full transition-all bg-base-200 z-10 p-4 ${open ? 'top-[100%]' : 'top-[-250%]'}`}>
        {
          NavLinks.map((nav) => {
            return <Link key={nav.name} to={nav.link} onClick={() => setOpen(false)} className='indent-1 w-full hover:text-accent'>{nav.name}</Link>
          })
        }
        <ResumeBtn />
      </div>
